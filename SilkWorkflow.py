...
def _split_surface_axis(grid, weights, axis, t):
    if axis == "u":
        ...
        right_weights = []
        for i in range(4):
            (left_curve, left_w), (right_curve, right_w) = _split_rational_bezier(
                grid[i], weights[i], t
            )
        ...
        for i in range(4):
            (left_curve, left_w), (right_curve, right_w) = _split_rational_bezier(
                trans_grid[i], trans_weights[i], t
            )
...
class SilkBoundarySpline(
    ...
    def __init__(self, obj, mode, payload):
        ...
        obj.addProperty(
            ...
            "SilkRole",
            "B1 - Boundary",
            "Silk workflow identifier",
        ).SilkRole = "SilkBoundarySpline"
        obj.addProperty(
            "App::PropertyString",
            "BoundaryMode",
            "B1 - Boundary",
        ...
        obj.addProperty(
            ...
            "ShowControlPoly",
            "B2 - Preview",
            "Display the control polyline",
        ).ShowControlPoly = False
        obj.addProperty(
            "App::PropertyBool",
            "ShowPreviewCurve",
            "B2 - Preview",
            "Overlay a cubic curve preview",
        ).ShowPreviewCurve = True
        obj.addProperty(
            "Part::PropertyPartShape",
            "PolyShape",
            "B3 - Cache",
            "Stored polyline shape",
        ).PolyShape = Part.Shape()
        obj.addProperty(
            "Part::PropertyPartShape",
            "CurveShape",
            "B3 - Cache",
            "Stored cubic curve preview",
        ).CurveShape = Part.Shape()
        obj.addProperty(
            "App::PropertyColor",
            "CurveColor",
            "B2 - Preview",
            "Preview curve display color",
        ).CurveColor = (1.0, 0.67, 0.0)
        obj.addProperty(
            "App::PropertyColor", "PolyColor", "B2 - Preview", "Polyline display color"
        ).PolyColor = (0.0, 1.0, 1.0)
        obj.addProperty(
            "App::PropertyString",
            "CreatorNote",
            "B1 - Boundary",
            "Free form comment about this boundary",
        ).CreatorNote = ""
        obj.addProperty(
            "App::PropertyPythonObject",
            "BlendSegments",
            "B4 - Blending",
            "Stored blend segments for this boundary",
        ).BlendSegments = []
        obj.addProperty(
            "App::PropertyInteger",
            "SegmentCounter",
    ...
    def _init_inputs(self, obj, payload):
        if self.boundary_mode == "3L":
            ...
            AN.ControlPoly4_2N.__init__(
                self, obj, payload["Sketch0"], payload["Sketch1"]
            )
            ...
            obj.addProperty(
                ...
                "Grid",
                "C1 - Inputs",
                "Reference grid or surface patch",
            ).Grid = payload["Grid"]
            obj.addProperty(
                "App::PropertyInteger",
                "EdgeIndex",
                "C1 - Inputs",
                "Grid edge index (0-3)",
            ).EdgeIndex = payload["EdgeIndex"]
            obj.addProperty(
                "App::PropertyFloatList", "Weights", "C1 - Inputs", "Weights"
            ...
            raise ValueError(
                "Unsupported BoundarySpline mode {}".format(self.boundary_mode)
            )
    ...
    def onDocumentRestored(self, obj):
        if not hasattr(obj, "SegmentCounter"):
            obj.addProperty(
                ...
                "B4 - Blending",
                "Next segment identifier",
            ).SegmentCounter = max(1, len(_segment_list(obj)))
        segs = _segment_list(obj)
        if not segs:
            obj.BlendSegments = [
                _create_segment_entry(obj, 0.0, 1.0, "Full", initial=True)
            ]
...
class CreateBoundarySplineCommand:
    ...
    def _interpret_selection(self):
        ...
        if len(sel) == 2:
            if (
                sel[0].Object.TypeId == "Sketcher::SketchObject"
                and sel[1].Object.TypeId == "Sketcher::SketchObject"
            ):
                return ("2N", {"Sketch0": sel[0].Object, "Sketch1": sel[1].Object})
            try:
                if (
                    sel[0].Object.object_type == "Point_onCurve"
                    and sel[1].Object.object_type == "Point_onCurve"
                ):
...
class SilkSurfacePatch(AN.ControlGrid44_4, AN.ControlGrid44_3):
    ...
    def _setup_common_properties(
        self, obj, boundaries, patch_type, update_topology=True
    ):
        ...
        if not hasattr(obj, "PatchTopology"):
            ...
            ).PatchTopology = self.PATCH_OPTIONS
        if update_topology:
        ...
        if not hasattr(obj, "ShowSurface"):
            obj.addProperty(
                ...
                "ShowSurface",
                "P2 - Display",
                "Display the underlying cubic surface",
            ).ShowSurface = True
        ...
        if not hasattr(obj, "ReverseNormal"):
            obj.addProperty(
                ...
                "ReverseNormal",
                "P2 - Display",
                "Flip the generated surface normal",
            ).ReverseNormal = False
        if not hasattr(obj, "GridShape"):
            obj.addProperty(
                ...
                "GridShape",
                "P3 - Cache",
                "Cached control grid shape",
            ).GridShape = Part.Shape()
        if not hasattr(obj, "SurfaceShape"):
            obj.addProperty(
                "Part::PropertyPartShape",
                "SurfaceShape",
                "P3 - Cache",
                "Cached cubic surface shape",
            ).SurfaceShape = Part.Shape()
    ...
    def _update_edge_segments(self, obj):
        if not hasattr(obj, "EdgeSegments"):
            obj.addProperty(
                ...
                "EdgeSegments",
                "P4 - Blending",
                "Per-edge blend segment references",
            ).EdgeSegments = {}
        ...
        for idx, name in enumerate(names):
            ...
            if boundary_obj is None:
                continue
            segments = []
            if hasattr(boundary_obj, "Proxy") and hasattr(
                boundary_obj.Proxy, "get_segments"
            ):
    ...
    def _edge_uses_reversed_boundary(self, obj, boundary, edge_index):
        ...
        if AN.equalVectors(boundary_poles[0], grid_edge[0], tol) and AN.equalVectors(
            boundary_poles[-1], grid_edge[-1], tol
        ):
            return False
        if AN.equalVectors(boundary_poles[0], grid_edge[-1], tol) and AN.equalVectors(
            boundary_poles[-1], grid_edge[0], tol
        ):
            return True
        return False
    ...
    def _current_builder(self):
        return (
            AN.ControlGrid44_3 if self.patch_topology == "Quad3" else AN.ControlGrid44_4
        )
    ...
    def get_edge_segment_geometry(
        self, obj, edge_index, segment_id, force_reverse=None
    ):
        names = self._poly_names_for()
...
class CreateSurfacePatchCommand:
    def GetResources(self):
        return {"Pixmap": "", "MenuText": "Silk SurfacePatch", "ToolTip": PATCH_TIP}
    ...
    def _collect_boundaries(self):
        ...
        if not selection:
            selection = [
                type("Sel", (object,), {"Object": o, "SubObjects": []})
        ...
        for sel in selection:
            ...
            if grid_edge:
                grid_obj, edge_idx = grid_edge
                new_boundary = _create_boundary_feature(
                    "GridEdge", {"Grid": grid_obj, "EdgeIndex": edge_idx}
                )
                created.append(new_boundary)
                bounds.append(new_boundary)
        if created:
...
class SilkBlendPatch:
    def __init__(self, obj, patch_a, patch_b):
        obj.addProperty(
            "App::PropertyLink", "PatchA", "C1 - Inputs", "First surface patch"
        ).PatchA = patch_a
        obj.addProperty(
            "App::PropertyLink", "PatchB", "C1 - Inputs", "Second surface patch"
        ).PatchB = patch_b
        obj.addProperty(
            "App::PropertyInteger", "EdgeIndexA", "C1 - Inputs", "Edge index on patch A"
        ).EdgeIndexA = 0
        obj.addProperty(
            "App::PropertyInteger", "EdgeIndexB", "C1 - Inputs", "Edge index on patch B"
        ).EdgeIndexB = 2
        obj.addProperty(
            "App::PropertyString",
            "SegmentIdA",
            "C1 - Inputs",
            "Segment identifier on patch A",
        ).SegmentIdA = ""
        obj.addProperty(
            "App::PropertyString",
            "SegmentIdB",
            "C1 - Inputs",
            "Segment identifier on patch B",
        ).SegmentIdB = ""
        obj.addProperty(
            "App::PropertyBool",
            "ReverseEdgeA",
            "C1 - Inputs",
            "Force reverse orientation on edge A",
        ).ReverseEdgeA = False
        obj.addProperty(
            "App::PropertyBool",
            "ReverseEdgeB",
            "C1 - Inputs",
            "Force reverse orientation on edge B",
        ).ReverseEdgeB = False
        obj.addProperty(
            "App::PropertyFloat",
            "TrimStartA",
            "C1 - Inputs",
            "Normalized trim start on boundary A",
        ).TrimStartA = 0.0
        obj.addProperty(
            "App::PropertyFloat",
            "TrimEndA",
            "C1 - Inputs",
            "Normalized trim end on boundary A",
        ).TrimEndA = 1.0
        obj.addProperty(
            "App::PropertyFloat",
            "TrimStartB",
            "C1 - Inputs",
            "Normalized trim start on boundary B",
        ).TrimStartB = 0.0
        obj.addProperty(
            "App::PropertyFloat",
            "TrimEndB",
            "C1 - Inputs",
            "Normalized trim end on boundary B",
        ).TrimEndB = 1.0
        obj.addProperty(
            "App::PropertyFloat",
            "ScaleTangentA",
            "C1 - Inputs",
            "Tangent scale for patch A",
        ).ScaleTangentA = 1.0
        obj.addProperty(
            "App::PropertyFloat",
            "ScaleTangentB",
            "C1 - Inputs",
            "Tangent scale for patch B",
        ).ScaleTangentB = 1.0
        obj.addProperty(
            "App::PropertyFloatList",
            "ScaleInnerA",
            "C1 - Inputs",
            "Inner scale for patch A",
        ).ScaleInnerA = [1.0, 1.0, 1.0, 1.0]
        obj.addProperty(
            "App::PropertyFloatList",
            "ScaleInnerB",
            "C1 - Inputs",
            "Inner scale for patch B",
        ).ScaleInnerB = [1.0, 1.0, 1.0, 1.0]
        obj.addProperty(
            "App::PropertyBool", "AutoG3", "C1 - Inputs", "Use G3 blending if supported"
        ).AutoG3 = False
        obj.addProperty(
            "App::PropertyBool",
            "EnableBlend",
            "C1 - Inputs",
            "Enable recompute of this blend",
        ).EnableBlend = False
        obj.addProperty(
            "App::PropertyVectorList", "Poles", "C2 - Outputs", "Blend control poles"
        ).Poles = []
        obj.addProperty(
            "App::PropertyFloatList", "Weights", "C2 - Outputs", "Blend weights"
        ).Weights = []
        obj.addProperty(
            "Part::PropertyPartShape", "GridShape", "C3 - Display", "Blend control grid"
        ).GridShape = Part.Shape()
        obj.addProperty(
            "Part::PropertyPartShape", "SurfaceShape", "C3 - Display", "Blend surface"
        ).SurfaceShape = Part.Shape()
        obj.Proxy = self
    ...
    def _build_manual_segment(self, patch, edge, segment, force_reverse=False):
        ...
        grid, wgrid = _trim_surface_interval(
            grid, wgrid, axis, segment["u_start"], segment["u_end"]
        )
    ...
    def execute(self, obj):
        ...
        if result_a is None or result_b is None:
            FreeCAD.Console.PrintError(
                "Blend segments not available on selected edges.\n"
            )
            return
    ...
    def debug_execute(self, obj):
        if not obj.EnableBlend:
            FreeCAD.Console.PrintMessage(
                "Blend execution disabled (EnableBlend = False).\n"
            )
            return
...
class CreateBlendPatchCommand:
    def GetResources(self):
        return {"Pixmap": "", "MenuText": "Silk SurfaceBlend", "ToolTip": BLEND_TIP}

    def Activated(self):
        boundaries = [obj for obj in Gui.Selection.getSelection() if _is_boundary(obj)]
        if len(boundaries) != 2:
            FreeCAD.Console.PrintError(
                "Select two Silk BoundarySplines (one on each patch).\n"
            )
            return
        info = [self._find_patch_info(bnd) for bnd in boundaries]
        if any(entry is None for entry in info):
            FreeCAD.Console.PrintError(
                "Could not find owning surface patches for selected boundaries.\n"
            )
            return
        if info[0][0] == info[1][0]:
            FreeCAD.Console.PrintError(
                "Select boundaries from two different surface patches.\n"
            )
            return
        doc = FreeCAD.ActiveDocument
        ...
        obj.SegmentIdA = ""
        obj.SegmentIdB = ""
        entry_a = (getattr(info[0][0], "EdgeSegments", {}) or {}).get(
            str(info[0][1]), {}
        )
        entry_b = (getattr(info[1][0], "EdgeSegments", {}) or {}).get(
            str(info[1][1]), {}
        )
...
class DebugTrimBoundary:
    ...
    def Activated(self):
        ...
        segment = {
            "u_start": getattr(boundary, "TrimStart", 0.0)
            if hasattr(boundary, "TrimStart")
            else 0.0,
            "u_end": getattr(boundary, "TrimEnd", 1.0)
            if hasattr(boundary, "TrimEnd")
            else 1.0,
        }
        poles, weights = boundary.Proxy.compute_segment_trim(boundary, segment)
...
class DebugPatchEdge:
    ...
    def Activated(self):
        ...
        if (
            len(selection) != 1
            or getattr(selection[0], "SilkRole", "") != "SilkSurfacePatch"
        ):
...
class DebugBlendSteps:
    ...
    def _run_steps(self, logger):
        ...
        logger.log(
            "Patch A: %s edge %d span %s"
            % (logger.patchA.Label, logger.edgeA, logger.trimA)
        )
        logger.log(
            "Patch B: %s edge %d span %s"
            % (logger.patchB.Label, logger.edgeB, logger.trimB)
        )
        ...
        for i in range(4):
            ...
            try:
                row = AN.blend_poly_2x4_1x6(
                    rows_a[i], weights_a[i], rows_b[i], weights_b[i], 1.0, 1.0, 1.0, 1.0
                )
